...
import (
	...
	"github.com/walkon/wsgnet/internal/socket"
	"github.com/walkon/wsgnet/internal/toolkit"
	"github.com/walkon/wsgnet/pkg/buffer/ring"
	"github.com/walkon/wsgnet/pkg/errors"
...
func (es *BuiltinEventEngine) PollerPreInit() {
}

func (es *BuiltinEventEngine) PollerProc() error {
	return nil
}

func (es *BuiltinEventEngine) PollerWaitTimeOut() int {
	return 10
}
...
func AddTCPConnector(eng *Engine, network, addr string, connIdx interface{}, opts ...Option) (c *conn, err error) {
	var (
		nfd int
		sa  unix.Sockaddr
		na  net.Addr
	...
	if options.TCPKeepAlive > 0 {
		sockopt := socket.Option{SetSockOpt: socket.SetKeepAlivePeriod, Opt: int(options.TCPKeepAlive / time.Second)}
		sockopts = append(sockopts, sockopt)
	}
	...
	nfd, na, sa, err = socket.TCPConnect(network, addr, sockopts...)
	if err != nil {
		return
	}

	el := eng.eng.lb.next(na)
	c = newTCPConn(nfd, el, sa, na, na)
